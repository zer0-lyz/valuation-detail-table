...
## 🚨 Step 3.pre: Phase 2e前置Gate检查（DT-161）

> **本步骤在Step 3.0之前执行，是Phase 3的入口检查。未通过=强制回退Phase 2e执行S2_seq_verify.md。**
...
# === Step 3.pre.2: 扫描往来科目Sheet的发生日期列 ===
...
# === Step 3.pre.3: 判定 ===
# 注意：使用人明确要求不填写发生日期时，Phase 2e已在Step 2e.0a中跳过并标注，
# 此处不再重复判定。若Phase 2e被跳过，往来科目发生日期列必然为空，
# 但has_journal_data=True且标注了"使用人明确要求(DT-161②)"时，Gate应通过。
# 此处通过读取缓存中的Phase 2e执行标注来判断：
user_skip_phase2e = False
phase2e_status_path = os.path.join(cache_dir, 'phase2e_status.json')
...
if checked_sheets == 0:
    print("✅ Gate通过: 无往来科目Sheet，无需Phase 2e")
elif not all_date_empty:
    print("✅ Gate通过: 往来科目发生日期已有值，Phase 2e已执行或部分执行")
elif user_skip_phase2e:
    print("✅ Gate通过: 使用人明确要求不填写发生日期(DT-161②)")
elif has_journal_data:
    print("🚨 GATE阻断(DT-161): 往来科目发生日期全空且项目有序时账数据")
    print("   → MUST回退执行S2_seq_verify.md Step 2e.0a~2e.9")
    print("   → 执行完毕后重新进入Phase 3，再次运行本Gate检查")
    # 回退执行Phase 2e后，Gate应通过
else:
    print("✅ Gate通过: 未提供序时账(DT-143)，发生日期留空")
```

**Gate未通过时的动作**（DT-161强制）：
1. 输出`🚨 Phase 2e GATE阻断`
2. **立即回退执行S2_seq_verify.md Step 2e.0a~2e.9**
3. Phase 2e执行完毕后重新进入Phase 3，再次运行Step 3.pre
4. 第二次仍不通过 → BLOCKED

**Step 3.pre.4: 业务内容质量校验（DT-149 + DT-60）**
...
for sname, info in re_biz_sheets.items():
    ...
    for r in range(7, ws.max_row + 1):
        ...
        if biz_val and str(biz_val).strip() in GENERIC_BIZ_CONTENTS:
            ...
            print(f"  ⚠️ 业务内容不达标 | {sname} Row{r} | 结算对象={str(name_val)[:20] if name_val else '?'} | 业务内容='{biz_val}' (DT-149: 通用模板文字，Phase 2e应从序时账摘要归纳)")

if biz_warnings > 0:
    print(f"\\n⚠️ 业务内容质量WARNING: {biz_warnings}行的业务内容为通用模板文字")
    if has_journal_data:
        print("   → 项目有序时账，Phase 2e Step 2e.5应从序时账摘要归纳业务内容(DT-60)")
    else:
...
### Step 3.0 操作结果断言（⚠️ Phase 2结束后强制执行，DT-114）
...
### Step 3.1 公式修复（⚠️ 降级为验证，DT-114闭环保障）
...
### Step 3.2 深度格式修复（⚠️ DT-114闭环保障，auto_fix_formats可自动执行3.2.1~3.2.4）
...
### Step 3.5 全量格式扫描 → 降级为validate_and_fix闭环
...
### Step 3.13 Phase 4.5 自动化验证门控（Phase 3完成后必须执行，未通过禁止进入Phase 4）
...
| 规则 | 编入步骤/自动工具 | 核心要点 |
|------|-----------------|---------|
| DT-2 插入行公式修复 | Step 3.1 → validate_and_fix | SUM范围+合计行引用+跨sheet引用 |
| DT-3 插入行复制参考行格式 | Step 3.1 → smart_insert_row | smart_insert_row自动copy格式 |
| DT-22 合计行A:C合并 | Step 3.6 | 统一合并A:C列+居中+thin边框 |
| DT-163 合计行B:C合并强制校验 | Step 3.6 | 检测跨行B:C合并扩展+重建缺失合并 |
| DT-24 删行公式修复 | Step 3.4 → smart_delete_rows | SUM收缩+跨sheet引用更新 |
| DT-25 空行直接删除 | Step 3.3 → smart_delete_rows | 禁止仅清边框 |
| DT-26 打印范围匹配 | Step 3.12 → smart_insert_row验证 | print_area精确覆盖内容 |
| DT-34 标题行合并单元格 | Step 3.7 | A1:H1/A2:H2恢复 |
| DT-35 合计行唯一性 | Step 3.8 → validate_detail_table | 每个明细表仅1个合计行 |
| DT-42 小计/减值行位置 | Step 3.9 | 紧贴合计行上方 |
| DT-43 跨sheet引用更新 | Step 3.10 → smart_insert_row | 含隐藏汇总表(DT-61) |
| DT-44 禁止空白无格式行 | Step 3.11 → smart_delete_rows | 数据行与合计行间无空白行 |
| DT-67 公式列禁止覆写 | Step 3.2.8 → auto_fix_formats检测 | J/K列MUST保留公式 |
| DT-76 增值额/增值率格式 | Step 3.2.2 → auto_fix_formats修复 | 特定数字格式强制 |
| DT-77 行高全表统一 | Step 3.2.1 → auto_fix_formats修复 | 同Sheet内数据行高一致 |
| DT-82 空白行格式补齐 | Step 3.2.4/3.2.7 | 边框+公式完整 |
| DT-83 合并单元格验证 | Step 3.2.5 | 对比模板merged_cells |
| DT-84 结构行A列居中 | Step 3.2.3 → auto_fix_formats修复 | 合计行/减值行A列居中 |
| DT-85 合计行公式引用 | Step 3.1 → validate_and_fix | 引用本行而非分隔行 |
| DT-86 跨sheet引用同步 | Step 3.10 → smart_insert_row | 汇总表+隐藏汇总表 |
| DT-112 格式集中处置 | Step 3.0~3.12 | 本Phase为格式集中处置区 |
| DT-114 验证-修复闭环 | Step 3.0/3.1/3.5 → validate_and_fix | 验证→修复→重验，最多3次 |
| DT-120 smart_insert_row强制 | Step 3.1/3.3 | 禁止裸insert_rows/delete_rows |
| DT-161 Phase 2e前置Gate | Step 3.pre | 往来科目发生日期全空且有序时账→强制回退Phase 2e |
